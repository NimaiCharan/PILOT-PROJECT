...
def get_ac_data_from_db(connection):
    try:
        cursor = connection.cursor()
        query = "select w.feels_like, ac.ac_mode FROM iq_ac_time_series ac, iq_weather_time_series w where ac_id='ACMF01' AND ac.date = w.date ANd ac.time = w.time;"
        cursor.execute(query)
        data = cursor.fetchall()
        headers= ["feels_like", "ac_mode"]
        df_data = pd.DataFrame(data, columns=headers)
        return df_data
    except Error as e:
        print(e)
